...
Provider OpenRouter officiel du runtime.
...
class OpenRouterProvider(BaseProvider):
    ...
    async def initialize(self):
        Logger.info(f"Initializing OpenRouter provider with model: {self.model_name}")

    async def _execute_http_request(self, payload: dict, endpoint: str = "https://openrouter.ai/api/v1/chat/completions") -> dict:
        ...
        while attempts < total_keys:
            ...
            headers = {
                ...
                "HTTP-Referer": "https://autocuse.ai",
                "X-Title": "AutoCUse Runtime"
            }
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(endpoint, headers=headers, json=payload) as response:
                        ...
                        if response.status in [429, 503] or "rate_limit" in error_text.lower() or "quota" in error_text.lower():
                            ...
                            Logger.warning(f"[OpenRouter Failover] Erreur {response.status} sur clé [{masked_key}...]. Rotation ({attempts}/{total_keys}).")
                            if self.has_available_keys():
                                await asyncio.sleep(0.5)
                                continue
                            raise ProviderQuotaExhaustedError(_("[OpenRouterProvider] Quota épuisé sur toutes les clés : {}").format(error_text))
                        elif response.status in [401, 403]:
                            self.mark_key_in_cooldown(active_key, 3600.0)
                            raise ProviderAuthError(_("[OpenRouterProvider] Clé invalide (401/403) : {}").format(error_text))
                        else:
                            raise ProviderError(_("OpenRouter API error {}: {}").format(response.status, error_text))
                ...
                if self.has_available_keys():
                    await asyncio.sleep(0.5)
                    continue
                raise ProviderServiceUnavailableError(_("OpenRouter network error: {}").format(str(ce))) from ce

        raise ProviderQuotaExhaustedError(_("[OpenRouterProvider] Quota épuisé sur toutes les clés."))
        ...
        while attempts < total_keys:
            ...
            try:
                    ...
                    async with session.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=payload) as response:
                        if response.status in [429, 503]:
                            ...
                            if self.has_available_keys():
                                await asyncio.sleep(0.5)
                                continue
                            raise ProviderQuotaExhaustedError(_("[OpenRouterProvider] Quota streaming épuisé : {}").format(error_text))

                        if response.status != 200:
                            error_text = await response.text()
                            raise ProviderError(f"OpenRouter streaming error: {error_text}")
                        ...
                        if is_tool_call:
                            ...
                            try:
                                ...
                                Logger.error(f"OpenRouter invalid JSON for tool args: {tool_args_str}")
                                parsed_args = {}
                ...
                if self.has_available_keys():
                    await asyncio.sleep(0.5)
                    continue
                raise ProviderServiceUnavailableError(f"OpenRouter stream network error: {ce}") from ce
